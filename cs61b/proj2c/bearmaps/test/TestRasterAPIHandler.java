...
public class TestRasterAPIHandler {
    ...
    public static int depthCalculator(Double wantedDepthResolution, Double mapResoltuionConstant) {
        int depth = 0;
        Double tempResolution = mapResoltuionConstant;
        while (tempResolution > wantedDepthResolution) {
            if (depth >= 7) {
                return 7;
            }
            else {
                System.out.println(tempResolution);
                tempResolution = tempResolution / Math.pow(2, depth);
                depth += 1;
            }
        }
        return depth;
    ...
    public void testRandom() {
        ...
        Double desiredDepthRatio = 0.000005;
        int depth = depthCalculator(desiredDepthRatio, mapResolutionConstant);
        List<Integer> teststepsizegrowth = new ArrayList();
        teststepsizegrowth.add(1);
        teststepsizegrowth.add(2);
        teststepsizegrowth.add(7);
        for (int i : teststepsizegrowth) {
            Double Stepsize = RasterAPIHandler.findStepSizeLon(i);
            print(Stepsize);
        }
    }
    @Test
    public void testgrid() {
        ...
        int xLow = 6;
        int xUper = 0;
        String[][] grid = new String[yLow - yUper + 1][xLow - xUper + 1];
