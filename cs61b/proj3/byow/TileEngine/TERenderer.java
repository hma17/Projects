...
public class TERenderer {
    ...
    public void renderAll(TETile[][] world, String tile) {
        int numXTiles = world.length;
        int numYTiles = world[0].length;
        StdDraw.clear(new Color(0, 0, 0));
        for (int x = 0; x < numXTiles; x += 1) {
        ...
        StdDraw.text(4, 47, text);
        StdDraw.text(47, 47, tile);
        StdDraw.show();
    }

    public void renderCenter(GameWorld.Position pos, TETile[][] world, String tile) {
        int numXTiles = world.length;
        int numYTiles = world[0].length;
        StdDraw.clear(new Color(0, 0, 0));
        for (int i = 0; i < numXTiles; i += 1) {
            for (int j = 0; j < numYTiles; j += 1) {
                if (world[i][j] == null) {
                    throw new IllegalArgumentException("Tile at position x=" + i + ", y=" + j
                            + " is null.");
                } else if (GameWorld.isInsideCircle(pos, 3, new GameWorld.Position(i, j))) {
