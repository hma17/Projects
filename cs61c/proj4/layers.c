          ...
          for (int fy = 0; fy < f_h; fy++) {
            ...
            for (int fx = 0; fx < f_w; fx++) {
              ...
              if (in_y >= 0 && in_y < in_height && in_x >= 0 && in_x < in_width) {
                ...
                if (f_d == 20) {
                  for (int fd = 0; fd < f_d / 4 * 4; fd += 20) {
                    __m256d temp = _mm256_loadu_pd((__m256d *) &(f_weights[((f_w * fy) + fx) * f_d + 0]));
                    __m256d temp_2 = _mm256_loadu_pd((__m256d *) &(in_weights[((in_width * in_y) + in_x) * in_depth + 0]));
                    __m256d mul = _mm256_mul_pd (temp, temp_2);
                    sum  = _mm256_add_pd(sum, mul);

                    temp = _mm256_loadu_pd((__m256d *) &(f_weights[((f_w * fy) + fx) * f_d + 4]));
                     temp_2 = _mm256_loadu_pd((__m256d *) &(in_weights[((in_width * in_y) + in_x) * in_depth + 4]));
                     mul = _mm256_mul_pd (temp, temp_2);
                    sum  = _mm256_add_pd(sum, mul); 

                     temp = _mm256_loadu_pd((__m256d *) &(f_weights[((f_w * fy) + fx) * f_d + 8]));
                     temp_2 = _mm256_loadu_pd((__m256d *) &(in_weights[((in_width * in_y) + in_x) * in_depth + 8]));
                     mul = _mm256_mul_pd (temp, temp_2);
                    sum  = _mm256_add_pd(sum, mul);

                    temp = _mm256_loadu_pd((__m256d *) &(f_weights[((f_w * fy) + fx) * f_d + 12]));
                     temp_2 = _mm256_loadu_pd((__m256d *) &(in_weights[((in_width * in_y) + in_x) * in_depth + 12]));
                     mul = _mm256_mul_pd (temp, temp_2);
                    sum  = _mm256_add_pd(sum, mul);

                     temp = _mm256_loadu_pd((__m256d *) &(f_weights[((f_w * fy) + fx) * f_d + 16]));
                     temp_2 = _mm256_loadu_pd((__m256d *) &(in_weights[((in_width * in_y) + in_x) * in_depth + 16]));
                     mul = _mm256_mul_pd (temp, temp_2);
                    sum  = _mm256_add_pd(sum, mul);                  
                  }
                double A[4] = {0, 0, 0, 0};
                _mm256_storeu_pd((__m256d *) A, sum); 
                actual_sum += A[0] + A[1] + A[2] + A[3];
              }
                else if (f_d == 16) {
                  for (int fd = 0; fd < f_d / 4 * 4; fd += 16) {
                    __m256d temp = _mm256_loadu_pd((__m256d *) &(f_weights[((f_w * fy) + fx) * f_d + 0]));
                    __m256d temp_2 = _mm256_loadu_pd((__m256d *) &(in_weights[((in_width * in_y) + in_x) * in_depth + 0]));
                    __m256d mul = _mm256_mul_pd (temp, temp_2);
                    sum  = _mm256_add_pd(sum, mul);
                ...
                actual_sum += A[0] + A[1] + A[2] + A[3];
                    ...
                    actual_sum += (f_weights[((f_w * fy) + fx) * f_d + 0]) * (in_weights[((in_width * in_y) + in_x) * in_depth + 0]);
                    actual_sum += (f_weights[((f_w * fy) + fx) * f_d + 1]) * (in_weights[((in_width * in_y) + in_x) * in_depth + 1]);
                    actual_sum += (f_weights[((f_w * fy) + fx) * f_d + 2]) * (in_weights[((in_width * in_y) + in_x) * in_depth + 2]);
                }
              }
            }
          }
        actual_sum += l->biases->weights[f];
        volume_set(out, out_x, out_y, f, actual_sum);
        }
      }
    }
  }
