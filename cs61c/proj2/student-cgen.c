 ...
 *  (the second is defined to be in s1). Very important with respect to the instructions.
 *  dispatch returns the s1 to the calculated value. This function sets up the right child
 *  child 1 to S1 and Child 0 to S2.
 *
...
void ProcessExprBinaryEq(DAST* dast,
  ...
  MatchSizes (child1, firstReg, child2, S1);
  /* STUDENT CODE HERE. */
  // EmitSLTU(T0, firstReg, S1);
  // EmitSLTU(T1, S1, firstReg);
  // EmitSLTIU(T0, T0, 1);
  // EmitSLTIU(T1, T1, 1);
  // EmitAND(S1, T0, T1);

  EmitSUB(S3, S1, firstReg);
  EmitSEQZ(S1, S3);
...
void ProcessExprBinaryAdd(DAST* dast,
  ...
  if(c1PtrTotal == 0 && c2PtrTotal == 0) {
    ...
    EmitLI(T0, child1->typeDecl->dataSize);
    EmitMUL(T0, T0, S1);
    EmitADD(S1, firstReg, T0);
  }
  else {
    EmitLI(T1, child2->typeDecl->dataSize);
    EmitMUL(T1, T1, firstReg);
    EmitADD(S1, S1, T1);
  }
...
void ProcessExprBinarySub(DAST* dast,
  ...
  else if (c1PtrTotal > 0 && c2PtrTotal == 0) {
    EmitLI(T0, child1->typeDecl->dataSize);
    EmitMUL(T0, S1, T0);
    EmitSUB(S1, firstReg, T0);
  }
  else if (c1PtrTotal == 0 && c2PtrTotal > 0){
    EmitLI(T1, child2->typeDecl->dataSize);
    EmitMUL(T1, T1, firstReg);
